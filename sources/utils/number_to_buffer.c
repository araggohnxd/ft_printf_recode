/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   number_to_buffer.c                                 :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: maolivei <maolivei@student.42sp.org.br>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/14 20:08:52 by maolivei          #+#    #+#             */
/*   Updated: 2023/06/15 18:50:34 by maolivei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int number_to_buffer(t_buffer *ctx, unsigned long num)
{
    char  buffer[NUMBER_BUFFER_SIZE];
    char *current;
    char *prefix;

    prefix  = ctx->meta.prefix;
    current = &buffer[NUMBER_BUFFER_SIZE - 1];
    while (1)
    {
        *--current = DIGITS[(num % ctx->meta.base) + ctx->meta.uppercase];
        num /= ctx->meta.base;
        if (num <= 0)
            break;
    }
    if (prefix)
        if (buffer_append(ctx, prefix, ft_strlen(prefix)) != 0)
            return (-1);
    return (buffer_append(
        ctx, current, (&buffer[NUMBER_BUFFER_SIZE - 1] - current)));
}
